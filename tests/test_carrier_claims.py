...
def engine():
    ...
    session.add_all([
        AutoPolicy(policy_id="C-001", person_id="P-A", holder_name="Alice",
                   state="CA", risk_score=70, status="active",
                   start_date=date(2023, 1, 1), premium=1200.0, carrier_name="GEICO"),
        AutoPolicy(policy_id="C-002", person_id="P-B", holder_name="Bob",
                   state="TX", risk_score=55, status="active",
                   start_date=date(2023, 1, 1), premium=900.0,  carrier_name="StateFarm"),
        AutoPolicy(policy_id="C-003", person_id="P-C", holder_name="Carol",
                   state="FL", risk_score=40, status="active",
                   start_date=date(2023, 1, 1), premium=700.0,  carrier_name="GEICO"),
    ])
    session.add_all([
        # GEICO: 2 claims in 2024, 1 in 2025
        Claim(claim_id="X-001", policy_id="C-001", person_id="P-A",
              claim_date=date(2024, 3, 1), claim_type="Collision", amount=5000, status="Closed"),
        Claim(claim_id="X-002", policy_id="C-003", person_id="P-C",
              claim_date=date(2024, 7, 1), claim_type="Theft",     amount=3000, status="Closed"),
        Claim(claim_id="X-003", policy_id="C-001", person_id="P-A",
              claim_date=date(2025, 2, 1), claim_type="Liability", amount=4000, status="Open"),
        # StateFarm: 1 claim in 2025
        Claim(claim_id="X-004", policy_id="C-002", person_id="P-B",
              claim_date=date(2025, 6, 1), claim_type="Weather",   amount=2000, status="Closed"),
    ])
    session.commit()
    session.close()
    return e
