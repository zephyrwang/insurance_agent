...
with st.sidebar:
    ...
    for domain, meta in AGENT_META.items():
        st.markdown(
            f'<span class="agent-badge {meta["badge"]}">' f'{meta["icon"]} {meta["label"]}</span>',
            unsafe_allow_html=True,
        )
...
if question_to_process and question_to_process.strip():
    ...
    # Add user message
    st.session_state.messages.append({"role": "user", "content": question, "id": _next_msg_id()})
    ...
    if load_error or not supervisor:
        ...
        with st.spinner("Thinking..."):
            try:
                ...
                st.session_state.routing_log.append({"question": question, "routed_to": domain})
